...
<!-- Ought to use #| in the first tutorial and tricks like suppress() and execute in the second. AR: done. -->

<!-- Add more about R Terminal/QMD world in the first example. It works well in the second already. Really ought to just switch the first example with the second! AR: Still in second, is that okay? -->

<!-- Should there be more projects, each moving more quickly during the creation phase, the core coding is done with AI? Only if this current tutorial only takes 45 minutes. AR: I can test and see how long this takes. -->
...
AR: How relevant is this comment still? Should I test how long current for takes, then decide? -->

<!-- More explicit emphasis on QMD world versus R Terminal world. AR: Two exercises in P2 do this now. -->

<!-- Consider getting rid of palmerpenguins. AR: use mpg in P2 - okay to keep penguins in P1 ?-->

<!-- AR: Future task? -> Compare 04-github-1 with this tutorial. Ask to find repeat concepts and how to level it up slightly between the two. -->
...
<!-- AR: Where should student "be" when running the tutorial? Currently have them running tutorial in one Codespace then doing projects in another connected to its GitHub repo.-->
...
<!-- AR: Is 15 exercises per project the right length? I can test and see -->
...
<!-- AR: Add photos? Show photos in previous tutorial -->
...
<!-- AR: We moved the commit to before `quarto publish` in both projects to make them consistent. P1 originally committed after publishing. Does this ordering make sense, or should I keep the post-publish commit? -->
...
<!-- AR: Okay to repeat repo and Codespace setup? No pics okay? -->
...
<!-- AR: Replaced palmerpenguins with `mpg` in Project 2 (mpg is built into ggplot2, no NAs, no extra library needed). Is `mpg` the right dataset here, or is there a better option for this context? -->
...
<!-- AR: The QMD World / R Terminal World concept is emphasized across Ex 10 and Ex 11. Is this the right amount of emphasis, or is it too much? -->
