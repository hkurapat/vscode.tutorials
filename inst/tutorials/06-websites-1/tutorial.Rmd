...
<!-- AR: No longer creating a README - is that okay? -->
...
If you look at the `_site` directory in the Explorer pane, you will see lots of new files, including `about.html`. Quarto just tells you about the home page for the site, which is `index.html` by default, since that is the file that browsers like Google Chrome always look for.
...
Look at the Source Control button. Note how many new files there are. Most of this is garbage which does not belong on GitHub.
...
In Codespaces, a pop-up notification should appear automatically offering to open the forwarded port in your browser. If it does not, click on the forwarded URL in the terminal command output.
