...
Use `dbListTables()` with `con_names` to see what tables are available in the `nameby_year.duckdb` database. Add this code in a new code chunk to the QMD. In a bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Generate SQL code that takes `babynames` and filters the dataset to show all names that have a first name starting with A after the year 2000. If necessary, change the code so that the result of the pipe is assigned to a variable called "sql". 
...
Get the results of the query by using `dbGetQuery()`. Add this code to the same code chunk in your QMD. In a bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Use `dbplyr` to filter the data to show all names starting with A after the year 2000. Paste this code into the same code chunk. In a bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Continue the pipe to classify names as "Short" or "Long" based on name length, where anything 5 characters or fewer is "Short". Add this code as a continuation of your pipe to the same chunk in your QMD. Since the `babynames` dataset contains names used for babies from 1880 to 2017 in America, we are using the truncated average length of an American name during 2009: 5.
...
Arrange the summarized results in ascending order by `year`, and collect the results from the database into a local R tibble for further use, such as plotting. Add the code to your QMD in the same chunk. In a bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Generate R code that plots a basic line graph showing the proportion of "Short" vs. "Long" names starting with a specific letter --- the letter you choose to replace `A`--- over the years.
...
Start a pipe from `con_air` to retrieve the contents of the `flights` table from the `nycflights13.duckdb` database using `dbReadTable()` and convert it to a tibble using `as_tibble()`. Make sure the result is not assigned to a variable. Add this code in a new code chunk to the QMD. In a bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Check the column type of `tailnum` from the `flights` table using `class()`. Add the code to your R Terminal and hit `Enter`.
...
Write code that summarizes how many missing values are in the `dep_time` and `arr_time` columns using the function `is.na()`. Add the code to your QMD in a new chunk. In a bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Continue your previous pipe and filters out rows where dep_time is missing using `!is.na()`. Add the code to your QMD in the same chunk as an addition to the pipe that is assigned to `departure_data`. In a bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Find tail numbers starting with "N1". Add the code to your QMD in a new code chunk. In a bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
In the R Terminal, run:
...
The scheduled departure time and the actual departure time for flights is written in the 24-hr system. Let's change this to the 12-hr system. Break military time into hours and minutes in R using integer division and modulo operators for the `sched_dep_time` and `dep_time` columns. Add the code to your QMD in a new code chunk. In a bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Generate R code that creates a beautiful heatmap. (You can make a different plot than a heatmap, if you like.) The plot must make sense. It must be something which tells us about the world. It can't just be nonsense. You need to look at the output.
