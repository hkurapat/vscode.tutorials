...
You will learn about working with numeric data and text patterns using core *Tidyverse* packages like **readr**, **dplyr**, and **stringr**, as well as database interaction using **dbplyr**. Key concepts include extracting numbers from strings, detecting text patterns, computing parallel maxima across vectors, concatenating and flattening strings, and connecting to databases.

You will also explore logical operations and concepts like integer division and modulo. This knowledge will help you manipulate numbers, strings, logical vectors, and connect with DuckDB databases using SQL from within R.
...
In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab — it should refresh automatically. In the R Terminal, run:
...
Key **DBI** and **dbplyr** concepts you'll use:

- Connecting to a database file and opening a named connection
- Reading a complete table from a database into a data frame
- Converting data frames to tibbles for better printing and consistency
- Creating a lazy reference to a database table so **dplyr** code translates to SQL
- Executing a raw SQL query and returning the results as a data frame
...
Ask AI: how to connect the `nameby_year.duckdb` database located in the "data" directory to `duckdb` using `DBI::dbConnect()` by assigning it to a variable `con_names`, so it is accessible to use with the rest of the code in the document. Add the code to your QMD in a new chunk. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Ask AI: How to use `dbListTables()` with `con_names` to see what tables are available in the `nameby_year.duckdb` database. Add this code in a new code chunk to the QMD. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Add the code as a replacement to the current code in your current chuck in the QMD. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Ask AI: how to get the results of the query by using `dbGetQuery()`. Add this code to the same code chunk in your QMD. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Ask AI: how to use `dbplyr` to filter the data to show all names starting with A after the year 2000. Paste this code into the same code chunk. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Ask AI for a continuation that classifies names as "Short" or "Long" based on name length, where anything 5 characters or fewer is "Short". Add this code as a continuation of your pipe to the same chunk in your QMD. Since the `babynames` dataset contains names used for babies from 1880 to 2017 in America, we are using the truncated average length of an American name during 2009: 5.
...
Ask AI to continue the pipe by using `.by()` to summarize the data by grouping by `year` and `length_class`, calculating the total number of babies in each group by summing `n` using `sum()`. Add the code as a continuation of your pipe to your QMD in the same chunk. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Ask AI to arrange the summarized results in ascending order by `year`, and collect the results from the database into a local R tibble for further use, such as plotting. Add the code to your QMD in the same chunk. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Within `labs()` edit or add a proper title, subtitle, and caption. If axis labels are appropriate, add them, but if unnecessary, don't bother. Don't assign the code to any variable. Put this code into a new code chunk. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Key **readr**, **stringr**, and base R concepts you'll use:

- Extracting the first number from a string, ignoring surrounding text
- Checking which values in a vector are missing
- Detecting whether a text pattern appears in each element of a string column
- Computing the element-wise maximum across two vectors (e.g., flooring negative values at zero)

We will also learn about integer division and modulo.
...
Ask AI: how to connect the `flights` table from the `nycflights13` database located in the "data" directory to `duckdb` using `DBI::dbConnect()` by assigning it to a variable `con_air`, so it is accessible to use with the rest of the code in the document. Add the code to your QMD in a new chunk. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Ask AI: how to start a pipe from `con_air` to retrieve the contents of the `flights` table from the `nycflights13.duckdb` database using `dbReadTable()` and convert it to a tibble using `as_tibble()`. Make sure the result is not assigned to a variable. Add this code in a new code chunk to the QMD. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Ask AI to write code that summarizes how many missing values are in the `dep_time` and `arr_time` columns using the function `is.na()`. Add the code to your QMD in a new chunk. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Ask AI: how you can continue your previous pipe and filters out rows where dep_time is missing using `!is.na()`. Add the code to your QMD in the same chunk as an addition to the pipe that is assigned to `departure_data`. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Ask AI: how you can find tail numbers starting with "N1". Add the code to your QMD in a new code chunk. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
In the R Terminal, run:
...
**stringr**'s pattern detection returns a logical vector of the same length as the input, which pairs well with `filter()`. Two related operations in **stringr** are: returning only the strings that match (subsetting), and returning the positions of matching strings (indexing).

Also, **dplyr** can count the number of distinct (unique) values of one or more variables, related to the same family of operations as `count()`.

### Exercise 10

The scheduled departure time and the actual departure time for flights is written in the 24-hr system. Let's change this to the 12-hr system. Ask AI how to break military time into hours and minutes in R using integer division and modulo operators for the `sched_dep_time` and `dep_time` columns. Add the code to your QMD in a new code chunk. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Note that computing the element-wise maximum across vectors is different from summary functions that collapse multiple observations into a single value. You can tell you’ve used the wrong form when all the minimums and all the maximums have the same value.
...
Ask AI to generate R code that creates a beautiful heatmap. (You can make a different plot than a heatmap, if you like.) The plot must make sense. It must be something which tells us about the world. It can't just be nonsense. You need to look at the output.

Within `labs()` edit or add a proper title, subtitle, and caption. If axis labels are appropriate, add them, but if unnecessary, don't bother. Don't assign the code for the plot to any variable. Add this code to a new code cell. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
