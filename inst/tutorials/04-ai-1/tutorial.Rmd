...
```
Create a .gitignore file for a Quarto R project. Include a pattern for build directories, but do NOT ignore .html files.
```
...
Keep chunks clean as you work. In these tutorials we iteratively rewrite a single working chunk rather than creating new ones --- a finished `analysis.qmd` is always short.
...
Throughout this tutorial we will often say "ask Gemini to" when we want you to use AI. In future tutorials, using AI to write your analysis code is always assumed --- we will not say so explicitly, but AI is how the work gets done.
...
Before caching a chunk, it should hold only the definition of the artifact to be cached --- no exploratory or intermediate steps.
...
Ask Gemini to update the chunk to filter `flights` to `origin == "JFK"` and select `dep_delay`, `arr_delay`, and `carrier`. Leave the result as the final expression so it prints. Render. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.
...
Assignment captures the result silently --- a chunk that only uses `<-` to assign a value to an objectproduces no output in the render. A blank section in the HTML is not an error; it means the chunk built an object without displaying it.
...
Calling a variable by name on the last line of a chunk is all it takes to print it. In `misc.tutorials`, ending a data-preparation chunk with the variable name is a standard way to confirm its shape before building a plot.
...
Every `analysis.qmd` follows a similar pattern to this: a setup chunk, and at least one chunk per topic that builds a final artifact. At least one of the topic chunks will be cached.

###

From here on, using AI to write your analysis code is always the assumed approach --- you will describe a goal, let the agent write the code, render, and inspect the result.
...
This confirms the full file --- YAML header, setup chunk, cached storms chunk, and JFK flights plot --- renders cleanly without Gemini running.
