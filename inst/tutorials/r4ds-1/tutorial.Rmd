...
If using a chat AI, first share a few rows of `billboard` from the R Terminal. In-environment agents can access your data directly.

Copy and paste the AI generated code into a new code chunk with your QMD. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Ask AI: Continue the code to add a smooth trend line and proper title, subtitle, caption, and axis labels.

Copy and paste this code to continue the previous code. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
If using a chat AI, start a new conversation and share a few rows of `billboard` from the R Terminal.
...
Ask AI: Continue the code to split the graph into separate panels, one per track.
...
For categorical variables, splitting your plot into **facets** creates subplots that each display one subset of the data. The variable you facet by should be categorical.

### Exercise 6

If using a chat AI, share your current code.

Ask AI: Continue the code by assigning the plot to the variable `line_graph`, and then save it to a file.

Update your code in `analysis.qmd`. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
This creates the plot object `line_graph` which you can view by typing its name in the R Terminal. **ggplot2** can save it as a PNG, PDF, or other image format.

### Exercise 7

If using a chat AI, start a new conversation and share a few rows of `billboard` from the R Terminal.

Ask AI: Create a box plot showing the distribution of ranks of 3 songs.
...
Ask AI: Create a bar chart showing the count of weeks 3 songs were ranked in the top 100.
...
If you're using a chat AI, provide the `billboard` tibble again and the previous code.

Copy and paste the AI generated code in the place to continue the previous code. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
Ask AI: Generate R code that uses `billboard_clean` to plot a basic graph showing a histogram for `improvement`. If using a chat AI, copy/paste a few rows of `billboard_clean` from the R Terminal to give it context. In-environment agents can access the data directly.
...
Ask AI: Continue the code to style the plot's appearance so the title and subtitle fit on the graph. If necessary, manually edit the sizes. Place this code as a continuation of the graph code; in the end the chunk should just have the final graph. In a Bash terminal, run `quarto render analysis.qmd` to ensure that everything works. Check your Live Server tab.
...
Ask AI: Edit the code so that `read_csv()` is assigned to an object `df` and told that the column type of `artist.familiarity` is a double. Then check `df` for any parsing failures.
...
Ask AI: Edit the code `read_csv("data/music.csv")` so that it only reads the columns `artist.familiarity`, `artist.hotttnesss`, and `artist.id`, overriding their types to be strings.
...
We are returning to our previous data frame `billboard`. If using a chat AI, share a few rows of `billboard` from the R Terminal to remind it of what we are working with.
...
Copy and paste the AI generated code to continue the previous code. In a Bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
...
This data is now tidy as NA values are removed, but we could make future computation a bit easier by converting values of week from character strings to numbers using `mutate()`. **readr** has a helpful tool that extracts the first number from a string, ignoring all other text.
...
Ask AI to generate R code that uses `billboard_longer` to plot a basic graph showing the Billboard rank of each track over the weeks after they were released. If using a chat AI, copy/paste a few rows of `billboard_longer` from the R Terminal to give it context. In-environment agents can access the data directly.
