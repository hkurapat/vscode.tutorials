...
You must do this tutorial in a Codespace created from a GitHub repo named `06-ai`. If you haven't already done this, follow these steps:

1. Create a GitHub Repo: go to the [codespace-starter repo](https://github.com/ppbds-student/codespace-starter). Click the green "Use this Template" drop-down button and select "Create a new repository". Name the repo `06-ai`. Then click "Create repository".

2. Create a Codespace from `06-ai`: from this repo's page, click the green "Code" drop down button, select the "Codespaces" tab, and click "Create Codespace on main".

Copy and paste the URL for your `06-ai` repository.
...
````
https://github.com/ppbds-student/06-ai
````
...
````
@ppbds-student ➜ /workspaces/06-ai (main) $ pwd
/workspaces/06-ai
@ppbds-student ➜ /workspaces/06-ai (main) $
````

Gemini CLI can see exactly what you can see. Because it runs in the same directory (`/workspaces/06-ai`), any files here are directly accessible to it --- a key difference from a chat interface, where you'd have to manually paste file contents for the AI to read them.
...
````
@ppbds-student ➜ /workspaces/06-ai (main) $ ls -a
.  ..  .devcontainer  .git
@ppbds-student ➜ /workspaces/06-ai (main) $
````
...
````
@ppbds-student ➜ /workspaces/06-ai (main) $ git log --oneline
...
```
https://ppbds-student.github.io/06-ai/
```
