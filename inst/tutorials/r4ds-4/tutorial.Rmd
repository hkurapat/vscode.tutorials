...
Ask AI to write a single-pipe R workflow to read the `line_score.parquet` file, parse the `game_date_est` column into a date, then extract year, month (with labels), and weekday (with labels). Convert `team_abbreviation_home` into a factor and rename old team codes (e.g., map "PHW" to "Warriors", "MNL" to "Lakers"), then lump all but the top 8 teams into "Other". Then create a `home_result` factor from `pts_home` vs `pts_away`.
...
Ask AI to generate R code that uses `nba_clean` to create an interesting plot.
...
Ask AI to write R code that opens the FIFA dataset, then begins cleaning with `mutate()` to create numeric height and weight columns by extracting the numeric digits from the text strings. Add this code as a replacement to your current code in the same code chunk in the same QMD.
...
Ask AI to write R code that continues our pipeline and cleans text formatting. Remove newline characters from all character columns, then in a second step, trim leading and trailing whitespace from character columns. Add this code as a continuation of your current pipe in the same code chunk in the same QMD.
...
Ask AI to use the `final_fifa` dataset to generate an interesting chart. Deal with any warnings or messages in a sensible fashion.
