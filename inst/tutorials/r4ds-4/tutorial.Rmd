...
In your QMD, put `library(tidyverse)`,`library(arrow)`, `library(plotly)`, `library(viridis)` and `library(scales)`  in a new code chunk. In a bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab — it should refresh automatically.
...
Key **arrow**, **lubridate**, and **dplyr** concepts include reading parquet files from disk, parsing datetime strings into proper date-time objects, creating ordered categorical variables, combining datasets with `left_join()`, and replacing missing values with defaults.
...
Write R code that pipes the file `data/game.parquet` and opens the parquet file using `open_dataset()`. Make sure this code is not assigned to a variable and the pipe starts with `data/game.parquet`. Add this code to a new code chunk in the  QMD.
...
Write R code that continues our pipe that opened `data/game.parquet` and selects the columns `game_id`, `game_date`, and `season_id`, then collects it into R. After that, ask it to parse `game_date` as a datetime, extract the year, month (with labels), and weekday (with labels), and then count how many games occurred on each weekday, arranging the result in descending order. Add this code as a continuation of your current pipe in the same code chunk in the same QMD. Your output should look something like:
...
Write R code that changes our pipe so that it parses `game_date` as a datetime, creates a new variable called `decade` by flooring the year to the nearest decade, then filters out missing decades, groups by decade, and summarises the total number of games plus the percentage of missing values for `reb_home`, `fg3m_home`, and `stl_home`. Make sure the AI uses `.groups` to drop the grouping at the end.
...
Write R code that changes our pipe and groups it by `team_name_home`, then calculates the average home points per team, sorts the results in descending order, selects the top 4 teams, and then reorders `team_name_home` as a factor sorted by `avg_pts` for cleaner plotting. Make sure the AI uses `.groups` to drop the grouping at the end.
...
**lubridate** lets you construct dates and datetimes from individual components like year, month, day, hour, minute, and second.
...
Write a single-pipe R workflow to read the `line_score.parquet` file, parse the `game_date_est` column into a date, then extract year, month (with labels), and weekday (with labels). Convert `team_abbreviation_home` into a factor and rename old team codes (e.g., map "PHW" to "Warriors", "MNL" to "Lakers"), then lump all but the top 8 teams into "Other". Then create a `home_result` factor from `pts_home` vs `pts_away`.
...
Continue our pipe from the last exercise and categorize `margin_category` based on the absolute point difference, creating an ordered factor with levels "Close", "Moderate", and "Blowout". Create an `era` factor using `game_year` with levels "Early NBA", "Classic Era", and "Modern". Also compute the start of each month, the number of days since January 1st, and a date one week ahead. Finally, arrange the dataset by `game_date`, limit to 1000 rows, and count games by `era` and `margin_category`.
...
Write R code that continues your pipe and creates three new columns: `game_date` converted to a proper datetime object, `game_year` extracted from `game_date`, and `season_label` constructed by combining parts of the `season_id`. Then filter to remove rows where `game_date` is missing and to keep only rows where `game_year` is 1946 or later.
...
Generate R code that uses `nba_clean` to create an interesting plot.
...
Key **stringr**, **lubridate**, **dplyr**, and **forcats** concepts include replacing text patterns, parsing dates in month-day-year format, applying conditional transformations, reordering factor levels by a numeric variable, and extracting substrings matching a pattern.
...
Write R code that opens `data/fifa.parquet` using `open_dataset()` and `collect()`, then uses `glimpse()` to examine the structure of the FIFA dataset. Make sure this code is not assigned to a variable and the pipe starts with `data/fifa.parquet`. Add this code to a new code chunk in the same QMD.
...
Write R code that opens the FIFA dataset and creates clean numeric versions of height and weight by extracting the numeric digits from the `Height` and `Weight` columns (which contain units as text). Create new columns called `height_cm` and `weight_kg`. Use `select()` to show only the original and new height/weight columns, then `slice_head()` to verify the cleaning worked. Add this code as a replacement to your current code in the same code chunk in the same QMD.
...
Write R code that opens the FIFA dataset and examines which columns contain newline characters. Use `select()` with `where()` and `is.character` to get only character columns, then use `summarise()` with `across()` to count how many values in each character column contain `"\\n"` using `str_detect()` and `sum()`. Add this code as a replacement to your current code in the same code chunk in the same QMD.
...
Write R code that opens the FIFA dataset and focuses on the `Joined` column to understand date formats. Use `select()` to get `Name`, `Club`, and `Joined` columns, then `slice_sample()` to look at 15 random rows so we can see the variety of date formats before parsing. Add this code as a replacement to your current code in the same code chunk in the same QMD.
...
Write R code that opens the FIFA dataset from `"data/fifa.parquet"` and parses the `Joined` column as dates (month-day-year format). Then calculate `years_at_club` by subtracting `joined_date` from today's date and dividing by 365.25. Create a logical column `long_tenure` for players with more than 10 years at a club. Use `filter()` to keep only players with `long_tenure == TRUE`, `select()` key columns such as `Name`, `Club`, `Joined`, and `years_at_club`, and `arrange()` the results in descending order of `years_at_club`.
...
Write R code that opens the FIFA dataset and converts the `Value` column from text to numeric. Handle "M" (multiply by 1,000,000), "K" (multiply by 1,000), and plain "€" values by detecting these patterns and cleaning the currency symbols. Create a `value_numeric` column, then use `select()` to show `Name`, original `Value`, and `value_numeric`, and `arrange()` by descending value to see the most valuable players. Add this code as a replacement to your current code in the same code chunk in the same QMD.
...
**forcats** supports reordering a factor by the `y` values associated with the largest `x` values — useful for making line chart legends match the order of lines at the right edge of the plot.

### Exercise 11

Write R code that opens the FIFA dataset, then begins cleaning with `mutate()` to create numeric height and weight columns by extracting the numeric digits from the text strings. Add this code as a replacement to your current code in the same code chunk in the same QMD.
...
**forcats** lets you rename specific factor levels while leaving all others unchanged. It will warn you if you accidentally refer to a level that doesn’t exist.

### Exercise 12

Write R code that continues our pipeline and cleans text formatting. Remove newline characters from all character columns, then in a second step, trim leading and trailing whitespace from character columns. Add this code as a continuation of your current pipe in the same code chunk in the same QMD.
...
**forcats** supports collapsing many levels into a smaller set by grouping multiple old levels under a single new name — useful when categories are too granular for analysis.


### Exercise 13

Write R code that continues our pipeline and adds date parsing with club tenure calculation. Parse `Joined` as a date (month-day-year format), calculate `years_at_club` using date arithmetic (divide by 365.25), and create a `long_tenure` logical column for players with more than 10 years. Also remove rows where date parsing failed. Add this code as a continuation of your current pipe in the same code chunk in the same QMD.
...
**forcats** provides several lumping strategies for combining rare factor levels into “Other.” The simplest progressively merges the smallest categories, always keeping “Other” as the smallest group.

### Exercise 14

Write R code that continues our pipeline and converts financial columns to numeric. Create `value_numeric`, `wage_numeric`, and `release_clause_numeric` columns by detecting whether each value contains "M" (millions), "K" (thousands), or a plain number, then cleaning the currency symbols and scaling accordingly. Add this code as a continuation of your current pipe in the same code chunk in the same QMD.
...
Use the `final_fifa` dataset to generate an interesting chart. Deal with any warnings or messages in a sensible fashion.
...
You learned about working with parquet files using **[arrow](https://arrow.apache.org/docs/r/)**, manipulating categorical variables with **[forcats](https://forcats.tidyverse.org/)**, handling date/time data with **[lubridate](https://lubridate.tidyverse.org/)**, managing missing values, and performing joins. Key concepts included opening large datasets from disk, reordering factor levels for plotting, parsing datetime strings, combining datasets with `left_join()`, and dropping rows with missing values.
