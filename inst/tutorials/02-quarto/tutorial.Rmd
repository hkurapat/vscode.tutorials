...
<!-- AR: Should we explicitly say this assumes students are already inside a GitHub Codespace running VS Code in the browser? That context matters before any Terminal/Explorer instructions. -->
...
<!-- AR: can we just delete exercise 1 or replace with a Codespaces analog? Does the student already has their git repo and Codespace set up for the tutorial set up by this point? -->
...
<!-- AR: This is a good first AI mention. Should we give students a concrete prompt, e.g. "What is the difference between quarto render and quarto preview?" -->
...
<!-- AR: In Codespaces, autosave/settings may differ. Should we tell students to use Cmd/Ctrl + S throughout, or configure autosave earlier? -->
...
<!-- AR: Should students actually look at the html code to see .qmd -> .html -> nice rendered document? -->
...
The `+` at the end of a **ggplot2** line means "add another layer to this plot." The first line creates the coordinate system and maps variables to aesthetics. `geom_point()` adds the layer that actually draws the points.

If you forget the `+`, R treats the next line as a new command instead of part of the plot.
...
<!-- AR: This is another strong gen AI exercise candidate: ask AI to improve labels/title/caption for the iris plot, then have students choose/edit the result. -->
...
<!-- AR: Good place to teach AI debugging: paste the YAML error into AI and ask what indentation mistake caused it. -->
...
surrounded by backticks. `Cmd/Ctrl + Shift + K`. Notice that, strictly speaking, you do not need to save the document before you render it. Quarto preview will automatically save a document when you try to render. But it's always a good idea to hit save, just to be on the safe side. 
...
<!-- AR: Should this require the integrated VS Code Chat Sidebar specifically, if available, so students practice the target environment rather than external AI tools? -->
...
<!-- AR: Maybe add a cautionary AI knowledge drop here: AI is useful, but students must run the code, read errors, remove install.packages(), and understand what they submit. -->
