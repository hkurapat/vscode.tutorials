...
The `.devcontainer` directory is what tells GitHub Codespaces how to set up this Codespace. Without it, GitHub would use a generic default setup with no customization. The presence of `.git` confirms this repo is tracked by Git.
...
The github.com/codespaces page is where you manage all your active Codespaces. Each Codespace runs on a remote computer that uses hours from your GitHub monthly limit --- free accounts receive a monthly allowance. Stop or delete Codespaces you're not using to conserve it.
...
The `image` field tells GitHub which pre-built setup to use when creating the Codespace. Think of it as choosing a computer that already has certain software installed before you even open it. `mcr.microsoft.com/devcontainers/universal:2` is a general-purpose setup made by Microsoft that comes with Python and several other programming languages already installed.
...
`updateContentCommand` runs a shell command each time the Codespace's contents are updated. Here it runs `pip install -r requirements.txt`, which installs all packages listed in `requirements.txt` --- connecting the two files directly. The two core data science packages are `pandas` (tabular data manipulation) and `matplotlib` (plotting), the Python equivalents of the tidyverse tools you've been using in R. There is also a companion field `postCreateCommand` for commands that should run only once when the Codespace is first created, rather than on every update.
...
A `.ipynb` file is a Jupyter notebook --- a document that mixes text, code, and output in a series of runnable cells, similar to a Quarto document.
...
In the `git log --oneline` output, `(HEAD -> main, origin/main)` on the same commit means your local branch and GitHub are perfectly up to date with each other. The Source Control graph shows the same thing visually --- a purple cloud icon on the commit indicates it has been pushed to GitHub. Both are telling you the same story; the graph just makes it easier to see at a glance.
...
The row should show the Codespace's status as **Stopped**. Stopped Codespaces are not running and do not consume compute hours, but they still occupy storage and can be restarted at any time. Deleting a Codespace frees the storage entirely but cannot be undone.
...
The blank template starts with no `.devcontainer` directory at all. GitHub falls back to a generic default setup with no customization. This is the starting point for any brand-new project where you want to configure the Codespace from scratch.
...
AI models know the `devcontainer.json` format well because it is a widely-used format with extensive documentation. The response is often longer than needed --- AI tends to include optional fields like `postCreateCommand` or `features` as a precaution. Reviewing and simplifying AI output is a normal part of using it for technical setup work.
...
A full rebuild discards the existing container and constructs a new one from scratch using the updated `devcontainer.json`. This is slower than a standard rebuild but guarantees the Codespace is set up exactly as specified. Because `ghcr.io/rocker-org/devcontainer/tidyverse:4` already has R pre-installed, R is available the moment the rebuild finishes --- no install scripts needed.
...
The `image` field names the pre-built setup used as the starting point for the container. `ghcr.io/rocker-org/devcontainer/tidyverse:4` is a pre-built image maintained by the [Rocker Project](https://rocker-project.org/) that comes with R 4.x, the tidyverse, and common data science packages already installed. Using a dedicated R image means no install scripts are needed --- R is ready when the container starts.
...
The `customizations` block is where tool-specific settings live. Different tools that support devcontainers --- VS Code, GitHub Codespaces, and others --- each read their own key inside `customizations`. The `vscode` key contains settings that only VS Code will read, which is what allows the same `devcontainer.json` to work cleanly with other editors that also support the devcontainer format.
...
Extension IDs in VS Code follow the format `publisher.extension-name`. `REditorSupport.r` is the official R language extension, published by the REditorSupport organization. It provides colored code, code completion, and inline output for R in VS Code --- the same extension you have been using throughout this course.
...
You may have noticed that this file contains lines beginning with `//`. Standard JSON does not allow comments, but `devcontainer.json` uses an extended version called **JSONC** (JSON with Comments) that does. The comments here are written for instructors and future maintainers --- they explain *why* each setting exists, not just what it does.
...
Rather than a generic starting setup, `codespace-starter` uses a custom image built and maintained by the PPBDS organization. The image is **locked** to a specific version (`0.2.1`) rather than using `:latest` --- this ensures every student gets the exact same setup regardless of when they open the Codespace. The image pre-installs everything the course needs: Quarto, `gh`, `tutorial.helpers`, `vscode.tutorials`, and several AI coding tools including `gemini` and `claude`.
...
This setting tells the R extension to use `arf` --- a fast R console --- instead of launching R directly. `arf` starts faster and handles pasted multi-line code more reliably than the default R terminal. Because it lives at a specific path inside the image, the path is written out directly here so VS Code can always find it.
...
By default, R opens plots in a separate window. In a Codespace that window would be invisible, since the R process runs on a remote computer. Setting `r.plot.useHttpgd` to `true` sends plots to the `httpgd` tool instead, which streams them into a panel inside VS Code that you can actually see.
...
Word wrap causes long lines to wrap to the next line visually rather than extending off the right edge of the screen. Without it, long comment blocks or URLs in code appear truncated, and students on small screens (like Chromebooks) may not notice that content is hidden to the right.
...
By default, VS Code opens files in "preview" mode --- the tab title appears in italics, and clicking a second file replaces the preview tab rather than opening a new one. Beginners frequently think their file disappeared. Setting this to `false` means every file opens in a permanent tab.
...
Your output should show the full contents of the `devcontainer.json` from `codespace-starter`, including fields like `image`, `features`, `customizations`, and any startup commands. Compare this to the Jupyter template's `devcontainer.json` you explored in the Jupyter Notebook Devcontainer section --- the two files use the same structure but produce completely different setups.
...
The `devcontainer.json` file is itself committed to the repository --- which means the Codespace configuration travels with the code. Anyone who forks your repo and opens a Codespace gets the exact same setup you designed. This is the same principle behind `renv.lock` for R packages or `requirements.txt` for Python: capture your setup in files tracked by Git so it can be reproduced by anyone, anywhere.
