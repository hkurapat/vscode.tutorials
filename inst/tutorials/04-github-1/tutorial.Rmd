...
<!-- Change First-Repo to `repo-1`. Then, update all the relevant PNG files. AR: repo now named 04-github-1 that's what's in the images. Okay? -->
...
<!-- Should we have a repo-3? Only if this tutorial only takes 45 minutes now, which I doubt. AR: Check timing, then decide? or close this out? -->
...
Change the title to "First Analysis" and add your name as the author. Save the file with `Cmd/Ctrl + S` as `analysis-1.qmd`.
...
We can stop the preview of `analysis-1.qmd` by typing `Ctrl + c` in the Terminal. Once we do, the Viewer tab is empty, so we can close the entire Secondary Activity Bar.
...
<!-- DK: Put something. Something about Netifly and other services. AR: done.-->
