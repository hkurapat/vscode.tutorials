...
The black dot next to "Untitled-1" indicates that there are unsaved changes in this file. Save the file, naming it `analysis-1.qmd`.
...
Click the Source Control button to bring up the Source Control pane. The only file listed is `analysis-1.qmd`, which is **U**ntracked.  
...
Render `analysis-1.qmd` with `Cmd/Cntrl + Shift + K`, or by clicking the "Preview" button.
...
README.md       analysis.html   analysis-1.qmd    analysis-1_files
dkane@macbook repo-2 % 
````

All the junk files that we don't really care about, and which are cluttering up the Source Control pane, are located in the `analysis-1_files` directory.

### Exercise 7

Quarto is "Watching files for changes" at the Terminal. If you change and then render `analysis-1.qmd`, Quarto will automatically display the updated HTML in the Viewer. It is watching `analysis.html`. You can't use this Terminal until you stop this process.
...
From the Terminal, run `ls -R analysis-1_files`. CP/CR.
...
````
dkane@macbook repo-2 % ls -R analysis-1_files
libs
...
Use `File -> New Text File` to create a new `.gitignore`. Add `analysis-1_files` and an empty line to the file. Save it in the highest level of the project, i.e., in the same location as `analysis-1.qmd`.

```{r}
include_graphics("images/second-repo-4.png")
```

There are now only three changed files: `analysis-1.qmd`, `analysis.html`, and `.gitgnore`.
